<script>
    ...
    const divClicked = (Key) => {
        ...
        navigator.clipboard.writeText(`https://laantungir.github.io/img_repo/${Key}`);
...
<div id="divMain">
    {#each Object.entries(jsonImg) as [Key, Value]}
        <div class="divBoxes">
            ...
            <button class="btnImg"
                on:click={() => {
                    window.open(`https://laantungir.github.io/img_repo/${Key}`, "_blank");
                }}
            >

                <img class="clsThumb" src={`https://laantungir.github.io/img_repo/${Key}`} alt="pic" />
            </button>
        </div>
